...
func (r *tableRepository) Get(req *dto.Pagination) (RepositoryResult, int) {
	...
	// Hitung total data
	if errCount := r.DB.Model(&entity.Table{}).Where("merchant_id = ?", req.MerchantID).Count(&totalRows).Error; errCount != nil {
		return RepositoryResult{Error: errCount}, totalPages
	}

	for i := range tables {
		tables[i].Table = helpers.TruncateString(tables[i].Table, 47)
	}
