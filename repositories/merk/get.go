...
func (r *merkRepository) Get(req *dto.Pagination) (RepositoryResult, int) {
	var products []entity.Merk
	...
	// Hitung total data
	if errCount := r.DB.Model(&entity.Merk{}).Where("merchant_id = ?", req.MerchantID).Count(&totalRows).Error; errCount != nil {
		return RepositoryResult{Error: errCount}, totalPages
	}

	for i := range products {
		products[i].MerkName = helpers.TruncateString(products[i].MerkName, 47)
	}
