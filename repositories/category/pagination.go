...
func (r *categoryRepository) Pagination(req *dto.Pagination) (RepositoryResult, int) {
	...
	// Hitung total data
	if errCount := r.DB.Model(&entity.Category{}).Where("merchant_id = ?", req.MerchantID).Count(&totalRows).Error; errCount != nil {
		return RepositoryResult{Error: errCount}, totalPages
	}

	for i := range products {
		products[i].CategoryName = helpers.TruncateString(products[i].CategoryName, 47)
	}
