...
func (r *voucherRepository) Get(req *dto.Pagination) (RepositoryResult, int) {
	var products []entity.Voucher
	...
	// Hitung total data
	if errCount := r.DB.Model(&entity.Voucher{}).Where("merchant_id = ?", req.MerchantID).Count(&totalRows).Error; errCount != nil {
		return RepositoryResult{Error: errCount}, totalPages
	}

	for i := range products {
		products[i].MerchantID = helpers.TruncateString(products[i].MerchantID, 47)
	}
