...
func (r *unitRepository) Get(req *dto.Pagination) (RepositoryResult, int) {
	var products []entity.Unit
	...
	// Hitung total data
	if errCount := r.DB.Model(&entity.Unit{}).Where("merchant_id = ?", req.MerchantID).Count(&totalRows).Error; errCount != nil {
		return RepositoryResult{Error: errCount}, totalPages
	}

	for i := range products {
		products[i].UnitName = helpers.TruncateString(products[i].UnitName, 47)
	}
