...
func (r *discountRepository) Get(req *dto.Pagination) (RepositoryResult, int) {
	...
	// Hitung total data
	if errCount := r.DB.Model(&entity.Discount{}).Where("merchant_id = ?", req.MerchantID).Count(&totalRows).Error; errCount != nil {
		return RepositoryResult{Error: errCount}, totalPages
	}

	for i := range discounts {
		discounts[i].DiscountName = helpers.TruncateString(discounts[i].DiscountName, 47)
	}
